			...
			<div class="box-content">
				<ul style="background: none repeat scroll 0 0 #eee;border: 0 none;border-radius: 0;box-shadow: none;color: #aaa;line-height: 34px; margin: 0;">
					<li style="display: inline-block;line-height: 20px;">
						<a class="btn btn-primary" href="{{route('permission.create')}}">添加权限</a>
					</li>
					<li style="display: inline-block;line-height: 20px;">
				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
    					...
    					<tr>
							...
							<td class="center">
								<a class="btn btn-warning" href="{{route('permission.edit', ['permission'=>$permission->id])}}">
									<i class="icon-edit icon-white"></i> 编辑
								</a>
								<input type="hidden" value="{{$permission->id}}">
								<span>
								<form action="{{route('permission.destroy', ['permission'=>$permission->id])}}" method="post" style="display: inherit;margin:0px;">
									{{ csrf_field() }}
            						{{ method_field('DELETE') }}
