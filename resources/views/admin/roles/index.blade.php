			...
			<div class="box-content">
				<ul style="background: none repeat scroll 0 0 #eee;border: 0 none;border-radius: 0;box-shadow: none;color: #aaa;line-height: 34px; margin: 0;">
					<li style="display: inline-block;line-height: 20px;">
						<a class="btn btn-primary" href="{{route('role.create')}}">添加角色</a>
					</li>
					<li style="display: inline-block;line-height: 20px;">
				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
    					...
    					<tr>
							...
							<td class="center">
								<a class="btn btn-warning" href="{{route('role.edit', ['role'=>$role->id])}}">
									<i class="icon-edit icon-white"></i> 编辑
								</a>
								<a class="btn btn-info" href="{{route('role.editPermission', ['id'=>$role->id])}}">
									<i class="icon-edit icon-white"></i> 权限设置
								</a>
								<input type="hidden" value="{{$role->id}}">
								<span>
								<form action="{{route('role.destroy', ['role'=>$role->id])}}" method="post" style="    display: inherit;margin:0px;">
									{{ csrf_field() }}
            						{{ method_field('DELETE') }}
